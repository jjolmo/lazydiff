...
# Close LazyDiff if running
if pgrep -f "LazyDiff" > /dev/null 2>&1; then
    echo "Closing LazyDiff..."
    osascript -e 'quit app "LazyDiff"' 2>/dev/null || true
    for i in $(seq 1 10); do
        pgrep -f "LazyDiff" > /dev/null 2>&1 || break
        sleep 1
    done
...
echo ""
echo "Done! LazyDiff $TAG installed."
echo "Launching LazyDiff..."
